...
def calc_maxmin(SINR, alpha):
    ...
    loss = torch.sigmoid(torch.div(0.3, SINR))

    sum_loss = torch.sum(loss) - torch.min(SINR)
    return sum_loss
...
if __name__ == '__main__':
    ...
    sio.savemat('output-minibatch-newloss.mat', {'pBest': np.transpose(output), 'pBest_test': np.transpose(output_test),
                                                                                   'loss_me': loss_p})
    torch.save(model, './maxmin-model')
    ...
    plt.savefig('./Loss of Max-min.jpg')
    plt.show()